...
```text
    webAppCreator [-[no]overwriteFiles] [-[no]ignoreExistingFiles] \
                  ...
                  [-junit pathToJUnitJar] [-[no]maven] [-[no]ant] [-[no]useLegacyJetty] moduleName
```
...
| -[no]useLegacyJetty]    | DEPRECATED: Use deprecated JettyLauncher instead of an external server. (defaults to OFF)
