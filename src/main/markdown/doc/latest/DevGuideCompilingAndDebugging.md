...
<p><img src="/web-toolkit/images/war_output_dir.png"/></p>
...
<p>You may have noticed that one of the generated files is named after your module, followed by a <code>.nocache.js</code> suffix. This is the GWT bootstrap file. Similar to the output subdirectory <code>war/&lt;app_name&gt;</code>, the name of this file is also controlled by the <code>rename-to</code> attribute in your module XML file. This file is responsible for choosing the correct version of your application to load for your client based on their browser and locale, or any other custom selection rule (see <a href="/web-toolkit/doc/{{ release_path }}/DevGuideCodingBasics#DevGuideDeferredBinding">Deferred Binding</a>). The various versions of your application compliant to each browser / locale are the <code>&lt;md5&gt;.cache.html</code> application files (discussed below).</p>

<p>The host HTML page references this file so that clients visiting your page first download the bootstrap, and the bootstrap script in turn figures out which browser environment it is running in and determines the appropriate version of your application to load. See the <a href="/web-toolkit/doc/{{ release_path }}/DevGuideOrganizingProjects#DevGuideBootstrap">documentation</a> on the bootstrap process for more details.</p>
