...
*   The location of the `&lt;`module`&gt;.nocache.js file relative to the host HTML page is not significant; the .nocache.js file attempts to locate files _relative to
...
In the GWT 1.4 bootstrap model, GWT expects to find all its files in the same path as the `&lt;`module`&gt;`.nocache.js file. For example, let's suppose
your GWT Module's name is `com.company.app.MyApp`, and that the host HTML file is at `http://host.domain.tld/myApp/index.html`. Further suppose that the
`&lt;`module`&gt;`.nocache.js and other files generated by the GWT compiler are in the same path as the host HTML file. With this setup, the following sequence of
requests will occur:
...
ct
URLs to additional resources, be sure that the resources are located in the appropriate directory. For example, if you remap your GWT files to '/gwt-files/', but your host page is
`/index.html`, then the code `GWT.getModuleBaseForStaticFiles() + &quot;myImage.png&quot;` will result in the URL `/gwt-files/myImage.png`.</p>
