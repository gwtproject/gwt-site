...
where they're compiled to JavaScript before being executed.

Tests run in [(legacy) development mode](DevGuideCompilingAndDebugging.html#DevGuideDevMode) are run as normal Java bytecode in a JVM.
...
If you instead decide to run the JUnit TestRunner from command line,
